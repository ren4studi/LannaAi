...
async def get_stars_balance():
    ...
    try:
        ...
        # Создаем SSL контекст с certifi
...
async def cmd_balance(message: Message):
    ...
    try:
        ...
        if balance is not None:
            ...
            # Если метод не работает, показываем альтернативную информацию
            await status_msg.edit_text(
                "❌ Не удалось получить баланс через API.\n\n"
                "📊 **Альтернативный способ:**\n"
                "1. Напишите @getmybot\n"
                "2. Отправьте /balance\n"
                "3. Там увидите баланс всех ботов\n\n"
                "📤 Для вывода используйте @send",
                parse_mode="Markdown"
            )
    except Exception as e:
    ...
    if data == "premium_info":
        ...
        try:
            ...
            await bot.send_invoice(
                ...
                title="💎 Премиум подписка AI Search",
...
async def main():
    print("=" * 60)
    print("🤖 TELEGRAM AI SEARCH BOT v2.0")
    print("=" * 60)
    print(f"\n✅ Бот токен: {BOT_TOKEN[:10]}...")
    print(f"✅ Админ ID: {ADMIN_IDS[0] if ADMIN_IDS else 'Не указан'}")
